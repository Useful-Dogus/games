...
- Completion criterion: `games.config.ts`에 3개 게임 메타데이터가 정의되고, `next.config.js`에 3개 슬러그 rewrite 규칙이 존재한다.
...
- Tasks that can run in parallel:
  - 2, 3, 4는 1 완료 후 병렬 가능
  - 7의 문서 정합성 점검 일부는 6 직전부터 선행 가능 (최종 반영은 6 이후)
